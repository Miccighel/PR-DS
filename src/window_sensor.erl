...
init(EventManager) ->
  ...
  io:format(LogSender, "~p~p~n", ["SENSORE FINESTRE: Sensore in esecuzione con identificatore: ", self()]),
...
terminate(Reason, _State) ->
  {ok, LogSender} = file:open("../log/Log_Sender.txt", [append]),
  io:format(LogSender, "~p~p~p~p~n", ["SENSORE FINESTRE: Il sensore con identificatore ", self(), " e stato terminato per il motivo: ", Reason]),
